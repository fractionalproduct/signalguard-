...
- ✅ **Phase 4 containment — RESOLVED: v1 discovery operates WITHIN the curated watchlist.** Discovery may **rank/prioritize which watchlist symbols** get the expensive TA deep-dive (the 10/day budget), but **off-watchlist movers are NOT admitted in v1** — the existing `ta-ingest` `off_watchlist` drop enforces this (security C2 / invariant #5; the risk engine's liquidity/sector/OTC eligibility gates are disabled on the curated-watchlist assumption). Admitting arbitrary discovered symbols is deferred to a future "widen containment" project that first re-enables those compensating eligibility gates. Net: discovery→analysis chaining is realized within safe bounds, no new risk surface. *(D4-B queue plumbing is built; this decision keeps its inputs watchlist-bounded.)*
